...
PLOT_TITLE = "R3-B1F_EH CO2 density with date axis"
OUTPUT_PATH = Path(__file__).with_name("adv1-1.png")
...
def main() -> None:
    ...
    sensor_data_by_timestamp: dict[int, float] = {}
    for day_index in range(FETCH_DAYS):
        ...
        for row in raw_data:
            ...
            sensor_data_by_timestamp[timestamp] = float(row[3])

    if not sensor_data_by_timestamp:
        print("This sensor is not connected")
        return

    sorted_timestamps = sorted(sensor_data_by_timestamp)
    timestamps = np.array(
        [dt.datetime.fromtimestamp(ts) for ts in sorted_timestamps],
        dtype=object,
    )
    date_numbers = np.array(mdates.date2num(timestamps), dtype=float)
    co2_values = np.array(
        [sensor_data_by_timestamp[ts] for ts in sorted_timestamps],
        dtype=float,
    )

    fig, ax = plt.subplots(figsize=(12, 6))
    ax.plot(date_numbers, co2_values, color="tab:blue")
